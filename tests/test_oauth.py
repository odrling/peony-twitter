...
class MockClient:
    ...
    async def post(self, _data=None, _headers=None, **kwargs):
        ...
        if _data is not None:
            with patch.object(
                oauth.aiohttp.payload, "BytesPayload", side_effect=dummy_func
            ):
...
def test_oauth1_signature(oauth1_headers):
    signature = oauth1_headers.gen_signature(
        method="GET",
...
def test_oauth1_signature_no_token():
    ...
    signature = headers.gen_signature(
        method="GET",
        url="http://whatever.com",
        params={"hello": "world"},
        skip_params=False,
...
def test_oauth1_sign(oauth1_headers):
    ...
    with patch.object(oauth.time, "time", return_value=t):
        random.seed(0)
        headers = oauth1_headers.sign(
            method="POST", url="http://whatever.com", data={"hello": "world"}
        )
    ...
    expected = (
        ...
        'oauth_version="1.0"'.format(
            nonce=nonce, signature=oauth.quote(signature), time=int(t)
        )
    )
...
def test_headers_options():
    client = oauth.OAuth1Headers(
        "", "", user_agent="Awesome app", compression=False, headers={"Custom": "abc"}
    )
...
async def test_oauth2_sign(oauth2_headers):
    with patch.object(
        oauth2_headers, "refresh_token", side_effect=dummy
    ) as refresh_token:
...
def test_raw_form_data():
    ...
    with patch.object(oauth.aiohttp.payload, "BytesPayload", side_effect=dummy_func):
        formdata = oauth.RawFormData(
            {"access_token": "a%20bc%25", "access_token_secret": "cba"},
            quote_fields=False,
        )
