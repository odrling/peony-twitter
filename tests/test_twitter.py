...
keys_oauth = {1: oauth1_keys, 2: oauth2_keys}

# test if the keys are in the environment variables
test_oauth = {i: all(key in os.environ for key in keys_oauth[i]) for i in keys_oauth}

oauth2_creds = "consumer_key", "consumer_secret"
oauth1_creds = *oauth2_creds, "access_token", "access_token_secret"

creds_oauth = {1: oauth1_creds, 2: oauth2_creds}


clients = {1: None, 2: None}

headers = {1: oauth.OAuth1Headers, 2: oauth.OAuth2Headers}
...
def client_oauth(key, event_loop):
    ...
    if clients[key] is None:
        creds = {
            k: os.environ[envk] for k, envk in zip(creds_oauth[key], keys_oauth[key])
        }
        ...
        return pytest.mark.asyncio(
            pytest.mark.twitter(
                pytest.mark.timeout(1800)(
                    pytest.mark.skipif(
                        not test_oauth[key], reason="no credentials found"
                    )(func)
                )
            )
        )
...
async def test_user_timeline(oauth1_client):
    req = oauth1_client.api.statuses.user_timeline.get(screen_name="twitter", count=20)
...
async def test_iterator_incorrect_data(oauth1_client):
    req = oauth1_client.api.users.show.get(
        screen_name="twitter", include_entities=False
    )
    responses = req.iterator.with_max_id()
...
async def test_upload_media(oauth1_client, medias):
    ...
    await oauth1_client.api.statuses.update.post(status="", media_ids=media.media_id)


@oauth1_decorator
async def test_upload_tweet(oauth1_client):
    status = (
        "%d Living in the limelight the universal dream "
...
async def test_upload_tweet_with_media_chunked(oauth1_client, medias):
    for media in (medias[key] for key in ("pink_queen", "bloom", "video")):
        ...
        await oauth1_client.api.statuses.update.post(
